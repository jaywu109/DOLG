"""Part of code is modified from pytorch-metric-learning
"""
import json
import torch
import numpy as np
...
class ImbalancedDatasetSampler(torch.utils.data.sampler.Sampler):
    """Samples elements randomly from a imbalanced dataset given weights path
    Arguments:
        dataset: dataset
        weights_path: json path stores with weights
    """

    def __init__(self, dataset, weights_path):
        self.num_samples = len(dataset)
        with open(weights_path, 'r') as f:
            self.weights_map = json.load(f)

        # weight for each sample
        weights = []
        for idx in range(len(dataset)):
            name, _ = dataset[idx]
            weights.append(self.weights_map[name])
...
class MPerClassSampler(torch.utils.data.sampler.Sampler):
    ...
    def __init__(self, dataset, m, batch_size=None, length_before_new_iter=100000):
        ...
        if self.batch_size is None:
            ...
            assert (self.length_of_single_pass >= self.batch_size), "m * (number of unique labels) must be >= batch_size"
    ...
    def __iter__(self):
        ...
        for _ in range(num_iters):
            ...
            for label in curr_label_set:
                ...
                idx_list[i:i + self.m_per_class] = safe_random_choice(t, size=self.m_per_class)
                i += self.m_per_class
