...
help_url = ''
img_formats = ['.bmp', '.jpg', '.jpeg', '.png', '.tif', '.tiff', '.dng', '.jfif', '.webp']
...
class LoadImages:  # for inference
    ...
    def __init__(self, path, names=None, img_size=640):
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.count += 1
            try:
                img0 = cv2.imread(path, cv2.IMREAD_COLOR)  # BG
            except Exception:
                img0 = np.zeros((128, 128, 3), dtype=np.uint8)
                print('Cannot handle {}'.format(path))
            if img0 is None:
                img0 = np.zeros((128, 128, 3), dtype=np.uint8)
                print('Cannot handle {}'.format(path))
