...
from yolov4_scaled.utils.torch_utils import (time_synchronized, fuse_conv_and_bn, model_info, scale_img, initialize_weights, select_device)
...
class Model(nn.Module):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            clone_b = b.clone().detach().numpy()
            clone_b[:, 4] += math.log(8 / (640 / s)**2)  # obj (8 objects per 640 image)
            clone_b[:, 5:] += math.log(0.6 / (m.nc - 0.99)) if cf is None else torch.log(cf / cf.sum())  # cls
            del b
            mi.bias = torch.nn.Parameter(torch.Tensor(clone_b).view(-1), requires_grad=True)
